...
std::string SessionManager::createSession(const char *model_path, void *options) {
  ...
  try {
    ...
    sessions_[session_id] = std::make_shared<SessionInfo>(std::move(session_info));
...
std::vector<Ort::Value> SessionManager::runInference(const std::string &session_id,
  ...
  // huji patch: hold the session via shared_ptr and release the mutex before
  // Run(). Ort::Session::Run is thread-safe for concurrent calls, and holding
  // the global mutex during Run serialized every inference in the process.
  ...
  Ort::Session *session = session_info->session.get();
  if (!session) {
  ...
  return session->Run(*run_opts, input_names_char.data(), input_tensors.data(), input_tensors.size(),
