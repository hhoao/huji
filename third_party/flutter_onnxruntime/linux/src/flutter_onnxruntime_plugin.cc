...
static void run_inference_async(FlutterOnnxruntimePlugin *self, FlMethodCall *method_call, FlValue *args);
...
static void flutter_onnxruntime_plugin_handle_method_call(FlutterOnnxruntimePlugin *self, FlMethodCall *method_call) {
  ...
  if (strcmp(method, "getPlatformVersion") == 0) {
    ...
    // huji patch: inference runs on a background thread; the method call is
    // answered asynchronously once it completes, so the platform (GTK main)
    // thread is never blocked by Ort::Session::Run.
    run_inference_async(self, method_call, args);
    return;
...
// huji patch: async runInference --------------------------------------------
//
// Upstream executes Ort::Session::Run inside the method call handler, i.e. on
// the platform (GTK main) thread. Sustained inference therefore froze input
// processing and every other plugin channel (upstream only moved inference off
// the main thread on Android/iOS/macOS). This fork keeps FlValue/GObject
// access on the platform thread, runs tensor clone + ORT Run on a spawned
// worker thread (TensorManager/SessionManager are mutex-protected), and
// responds via fl_method_call_respond from a g_idle_add callback.

struct RunInferenceRequest {
  std::string session_id;
  // (input name, tensor id) pairs in call order
  std::vector<std::pair<std::string, std::string>> inputs;
  bool has_run_options = false;
  int log_severity_level = 0;
  int log_verbosity_level = 0;
  bool terminate = false;
};

struct RunOutputDesc {
  std::string value_id;
  std::string tensor_type;
  std::vector<int64_t> shape;
};

struct RunInferenceResult {
  bool success = false;
  std::string error_code;
  std::string error_message;
  std::vector<std::string> output_names;
  std::vector<RunOutputDesc> outputs;
};

struct RunInferenceJob {
  FlutterOnnxruntimePlugin *plugin; // strong ref held while the job is alive
  FlMethodCall *method_call;        // strong ref held while the job is alive
  RunInferenceRequest request;
};

struct RunInferenceIdleData {
  FlutterOnnxruntimePlugin *plugin;
  FlMethodCall *method_call;
  RunInferenceResult *result; // owned
};

static void run_inference_job(RunInferenceJob *job) {
  auto *result = new RunInferenceResult();

  try {
    result->output_names = job->plugin->session_manager->getOutputNames(job->request.session_id);
    ...
    for (const auto &input : job->request.inputs) {
      try {
        // Clone the tensor — ClonedTensor owns both the Ort::Value and its backing buffer
        cloned_inputs.push_back(job->plugin->tensor_manager->cloneTensor(input.second));
        input_names.push_back(input.first);
      } catch (const std::exception &e) {
        g_warning("Failed to clone tensor %s: %s", input.second.c_str(), e.what());
    ...
    if (job->request.has_run_options) {
      run_options.SetRunLogSeverityLevel(job->request.log_severity_level);
      run_options.SetRunLogVerbosityLevel(job->request.log_verbosity_level);
      if (job->request.terminate) {
    ...
    if (!input_tensors.empty()) {
      output_tensors = job->plugin->session_manager->runInference(job->request.session_id, input_tensors,
                                                                   input_names, &run_options);
    }

    // For each output tensor, directly store it using TensorManager's storeTensor
    for (size_t i = 0; i < output_tensors.size(); i++) {
      // Create a tensor ID
      std::string value_id = job->plugin->tensor_manager->generateTensorId();

      // Store the tensor directly using storeTensor - this transfers ownership
      job->plugin->tensor_manager->storeTensor(value_id, std::move(output_tensors[i]));

      RunOutputDesc desc;
      desc.value_id = value_id;
      desc.tensor_type = job->plugin->tensor_manager->getTensorType(value_id);
      desc.shape = job->plugin->tensor_manager->getTensorShape(value_id);
    ...
    result->error_code = "PLUGIN_ERROR";
    result->error_message = e.what();
  }

  // Marshal the reply back to the platform thread: FlValue and the method
  // call response must only be touched on the GTK main loop.
  auto *idle_data = new RunInferenceIdleData{
      job->plugin,
      job->method_call,
      result,
  };
  ...
  g_idle_add_full(
      ...
      [](gpointer user_data) -> gboolean {
        auto *data = static_cast<RunInferenceIdleData *>(user_data);
        g_autoptr(FlMethodResponse) response = nullptr;

        if (data->result->success) {
          g_autoptr(FlValue) outputs_map = fl_value_new_map();
          const auto &names = data->result->output_names;
          const auto &outputs = data->result->outputs;
          for (size_t i = 0; i < outputs.size(); i++) {
            // Note: Flutter does not allow return a nested map, so we have to
            // use list here to keep the output_info format
            ...
            fl_value_append_take(output_info, fl_value_new_string(outputs[i].value_id.c_str()));
            fl_value_append_take(output_info, fl_value_new_string(outputs[i].tensor_type.c_str()));
            fl_value_append_take(output_info, shape_list);

            const std::string output_name =
                i < names.size() ? names[i] : ("output_" + std::to_string(i));
            fl_value_set_string_take(outputs_map, output_name.c_str(), output_info);
          }
          response = FL_METHOD_RESPONSE(fl_method_success_response_new(outputs_map));
        } else {
          response = FL_METHOD_RESPONSE(fl_method_error_response_new(
              data->result->error_code.c_str(), data->result->error_message.c_str(), nullptr));
        }

        if (!fl_method_call_respond(data->method_call, response, nullptr)) {
          g_warning("Failed to send runInference response");
        }
...
static void run_inference_async(FlutterOnnxruntimePlugin *self, FlMethodCall *method_call, FlValue *args) {
  // Everything below only reads FlValue on the platform thread.
  auto *job = new RunInferenceJob();
  job->plugin = FLUTTER_ONNXRUNTIME_PLUGIN(g_object_ref(self));
  job->method_call = FL_METHOD_CALL(g_object_ref(method_call));

  auto respond_error = [method_call](const char *code, const std::string &message) {
    g_autoptr(FlMethodResponse) response =
        FL_METHOD_RESPONSE(fl_method_error_response_new(code, message.c_str(), nullptr));
    if (!fl_method_call_respond(method_call, response, nullptr)) {
      g_warning("Failed to send runInference error response");
    }
  };
  auto cleanup_job = [job]() {
    g_object_unref(job->plugin);
    g_object_unref(job->method_call);
    delete job;
  };
  ...
  if (session_id_value == nullptr || fl_value_get_type(session_id_value) != FL_VALUE_TYPE_STRING) {
    respond_error("INVALID_ARG", "Session ID must be a non-null string");
    cleanup_job();
    return;
  }
  job->request.session_id = fl_value_get_string(session_id_value);
  ...
  if (inputs_value == nullptr || fl_value_get_type(inputs_value) != FL_VALUE_TYPE_MAP) { // inputs is a map from Dart
    respond_error("INVALID_ARG", "Inputs must be a non-null map");
    cleanup_job();
    return;
  }

  // Extract (input name, tensor id) pairs
  ...
  for (size_t i = 0; i < num_inputs; i++) {
    ...
    job->request.inputs.emplace_back(fl_value_get_string(key), fl_value_get_string(tensor_id_map));
  }
  ...
  if (run_options_value != nullptr && fl_value_get_type(run_options_value) == FL_VALUE_TYPE_MAP) {
    job->request.has_run_options = true;
    ...
    if (log_severity_level_value != nullptr && fl_value_get_type(log_severity_level_value) == FL_VALUE_TYPE_INT) {
      job->request.log_severity_level = static_cast<int>(fl_value_get_int(log_severity_level_value));
    }
    ...
    if (log_verbosity_level_value != nullptr && fl_value_get_type(log_verbosity_level_value) == FL_VALUE_TYPE_INT) {
      job->request.log_verbosity_level = static_cast<int>(fl_value_get_int(log_verbosity_level_value));
    }
    ...
    if (terminate_value != nullptr && fl_value_get_type(terminate_value) == FL_VALUE_TYPE_BOOL) {
      job->request.terminate = fl_value_get_bool(terminate_value);
    }
  }

  // Check if session exists (runInference re-validates on the worker thread)
  if (!self->session_manager->hasSession(job->request.session_id)) {
    respond_error("INVALID_SESSION", "Session not found");
    cleanup_job();
    return;
  }

  std::thread(run_inference_job, job).detach();
}
