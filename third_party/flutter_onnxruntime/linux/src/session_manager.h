...
private:
  ...
  //
  // huji patch: shared_ptr so an in-flight runInference() can hold the session
  // alive after closeSession() erased it from the map (runs execute outside
  // the mutex on a background thread).
